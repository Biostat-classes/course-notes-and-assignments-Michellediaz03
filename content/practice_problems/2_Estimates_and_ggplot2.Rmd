...
```{r}
ggplot(sleep,aes(x=BrainWt,y = TotalSleep)) + geom_point(size = 4)
  ylab("Average hours spent in sleeping daily") +
  xlab("Brain weight (g)") +
  ggtitle("Time spent sleeping \n decreases with brain \n weight") +
  #theme sets sizes, text, etc
```

```{ggplot(sleep, aes(x=BrainWt, y=TotalSleep)+ geom_point}
```
...
A 95% confidence interval indicates that the true value of the
population parameter will fall within the calculated interval in 95%.
